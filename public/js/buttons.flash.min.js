...
d.text&&c.appendChild(a.createTextNode(d.text)));
		...
		function y(a,b){
			...
			for(var e=0,f=a.body.length;e<f&&!(c=a.body[e][b].toString().length,c>d&&(d=c),40<d);e++);
				return 5<d?d:5}function u(a){n===o&&(n=-1===v.serializeToString(g.parseXML(p["xl/worksheets/sheet1.xml"])).indexOf("xmlns:r"));g.each(a,function(b,d){if(g.isPlainObject(d))u(d);else{if(n){var c=d.childNodes[0],
